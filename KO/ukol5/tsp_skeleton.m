...
function apply2optHeuristicsDCV(circuit, locations)
    ...
    while(retry)
        ...
        for j=(i+1):n
           ...
           Tn(:,(i+1:j-1)) = fliplr(flipud(T(:,(i+1:j-1))));
           
           % spocitaj cenu novej kruznice
           w = 0;
           for(k = 1:n)
               w = w+W(Tn(1,k), Tn(2,k));
           end
    ...
    % pridaj na koniec ceny, lebo inak to havaruje ...
    circuit = T;
    for(k = 1:n)
        circuit(3,k) = W(Tn(1,k), Tn(2,k));
    end
