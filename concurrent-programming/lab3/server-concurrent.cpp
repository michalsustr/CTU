...
#	endif
	...
	while( 1 )
	{
		...
		for( size_t i = 0; i < connections.size(); ++i )
		{
			switch( connections[i].state )
			{
				case eConnStateSending:
					FD_SET( connections[i].sock, &write_fd_set );
					break;

				case eConnStateReceiving:
					FD_SET( connections[i].sock, &read_fd_set );
					break;
			}
			maxfd = std::max( connections[i].sock, maxfd );
		}

                // put the accepting server to the reading set
                FD_SET(listenfd, &read_fd_set);
                maxfd = std::max(maxfd, listenfd)+1;

                /* Block until input arrives on one or more active sockets. */
                if (select (maxfd, &read_fd_set, &write_fd_set, NULL, NULL) < 0)
                {
                  perror ("select");
                  exit (EXIT_FAILURE);
                }

                // Process the accepting server
        ...
        #			endif
                    ...
                    for( size_t i = 0; i < connections.size(); ++i )
                    {
                        if( FD_ISSET( connections[i].sock, &write_fd_set ) ) {
                        ...
                        } else if( FD_ISSET( connections[i].sock, &read_fd_set ) ) {
                            bool processFurther = true;
...
static int setup_server_socket( short port )
{
	...
	char actualBuff[128];
	printf( "Socket is bound to %s %d, socket fd %d\n",
