...
public class TSPIndividual {
    ...
    public TSPIndividual[] orderCrossover(TSPIndividual mate) {
        ...
        int a = rand.nextInt(genes.length - 4);
        int b = a + 4;

        children[0] = this.clone();
        children[1] = mate.clone();

        int[] o1 = children[0].genes;
        int[] o2 = children[1].genes;
        int[] p1 = this.genes;
        int[] p2 = mate.genes;
        ...
        int[] remaining1 = new int[genes.length - (b - a)];
        int[] remaining2 = new int[genes.length - (b - a)];
        boolean add1, add2;
        // take the remaining genes
        int k1 = 0, k2 = 0;
        for (int i = b; i < genes.length; i++) {
            add1 = true;
            add2 = true;
            for (int j = a; j < b; j++) {
                if (o1[j] == p1[i]) {
        ...
        k1 = 0;
        k2 = 0;
        for (int i = b; i % genes.length < a; i++) {
            o1[i] = remaining1[k1++];
            o2[i] = remaining2[k2++];
        }
    ...
    public void swapMutation() {
        Random rand = new Random();
        int city1 = rand.nextInt(genes.length);
        int city2 = rand.nextInt(genes.length);
        int tmp;
        tmp = genes[city1];
        genes[city1] = genes[city2];
        genes[city2] = tmp;
    }
