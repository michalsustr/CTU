...
public class FeedforwardANN {
    ...
    private void propagate(double[] x) {
        for (int l = 0; l < getNumLayers(); l++) {
            ...
            for (int to = 0; to < getNumNeurons(l); to++) {
                // the last layer doesn't have bias
                if(l != getNumLayers()-1 && to == 0) {
                    continue;
                }

                // the zero input layer
                if(l == 0) {
                    setActivity(0, to, x[to-1]);
                } else {

                    // next layers
                    double ws = 0.0;
                    for (int from = 0; from < getNumNeurons(l-1); from++) {
                        ws+=getWeight(l, to, from)*getActivity(l-1, from);
                    }
                    setActivity(l, to, sigmoid(ws));
                }
            }
        }
    ...
    private static double sigmoid(double x) {
        return 1 / (1+Math.exp(-SIGMOID_GAIN*x));
    }
    ...
    private static double sigmoidDerivative(double y) {
        return SIGMOID_GAIN * sigmoid(y) * (1- sigmoid(y));
    }
    ...
    public void learnBP(double[][] trainX, double[][] trainY) {
        double[][][] w = allocateDeltaWeight();

        for (int l = 0; l < getNumLayers(); l++) {
            for (int n = 0; n < getNumNeurons(l); n++){
