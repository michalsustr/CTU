...
LandmarksCountHeuristic::LandmarksCountHeuristic(const LandmarksGraph& l,
		const BestFirstSearchEngine& s, bool preferred_ops,	FFHeuristic *ff_heur, 
    bool _use_metric) : Heuristic(_use_metric, false, false, LAMA), lgraph(l), sengine(s)
	{
...
int LandmarksCountHeuristic::compute_heuristic(const State &state) {
	...
	int h;
	step_cost = lgraph.number_of_landmarks() - reached_lms.size()	+ needed_lms.size();
  h = step_cost;
	if (use_metric)
  ...
  if (goal_reached && h != 0) {
	...
	if (reached_lms_cost == lgraph.cost_of_landmarks() || !generate_helpful_actions(state, reached_lms)) {
...
void LandmarksCountHeuristic::collect_lm_leaves(bool disjunctive_lms, hash_set<
		const LandmarkNode*, hash_pointer>& reached_lms,
...
bool LandmarksCountHeuristic::check_node_orders_disobeyed(
		const LandmarkNode& node, const hash_set<const LandmarkNode*,
