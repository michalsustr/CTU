	...
	bool operator<(const UnaryOperator &other) const {
		if (*(other.effect) < *effect)
			...
			while (i != precondition.size()) {
				if (i == other.precondition.size() || *(other.precondition[i])
						< *(precondition[i]))
					return false;
...
class FFHeuristic: public Heuristic {
	...
	typedef __gnu_cxx ::hash_set<const Operator *, hash_operator_ptr>
			RelaxedPlan;
	...
	void setup_exploration_queue(const State &state, const std::vector<
			std::pair<int, int> >& excluded_props, const __gnu_cxx ::hash_set<
			const Operator *, hash_operator_ptr>& excluded_ops, bool use_h_max);
	...
	void collect_ha(Proposition *goal, RelaxedPlan &relaxed_plan,
			const State &state);

	void enqueue_if_necessary(Proposition *prop, int cost, int depth,
...
public:
  int id;
	...
	void
			compute_reachability_with_excludes(
					std::vector<std::vector<int> >& lvl_var, std::vector<
							__gnu_cxx ::hash_map<pair<int, int> , int,
	...
	int plan_for_disj(std::vector<std::pair<int, int> >& disj_goal,
			const State& state);
	FFHeuristic(bool use_metric = false, bool use_cache = false, int is_mrw = 0);
	~FFHeuristic();
};
