...
public:
	...
	const Entry *update(const Entry &entry, const Entry *predecessor,
			const Annotation &annotation);
	int size() const;
	void trace_path(const Entry &entry, std::vector<Annotation> &path) const;
	const Entry * trace_one_step(const Entry &entry, std::vector<Annotation> &path) const;
	void trace_state_path(Entry &entry, std::vector<Entry*> &path) const;
