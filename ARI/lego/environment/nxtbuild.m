...
function sys= nxtbuild(varargin)
% check arguments
...
switch nargin
    ...
    otherwise
        error('NXTBuild:requires 1 or 2 arguments.')
end
...
switch cmd
    case 'cgen'
        ...
        rtwbuild([gcs_new '/' sys], 'Mode', 'ExportFunctionCalls');
        % check whether the folder name is correct
        renamed_dir=dir ([sys '*_ert_rtw']);
        rrr=regexp(renamed_dir.name,[ '(?<sys>' sys '.+)_ert_rtw'],'names');
        if not(isempty(rrr))
            sys = rrr.sys;
        end
        
        %disp(pwd)
        %if not(isempty(renamed_dir))
        %   [~, ~, ~]= rmdir([sys '_ert_rtw'],'s');
        %    
        %    unix( [ 'mv ' renamed_dir.name '   ' [' ' sys ,'_ert_rtw'] '']);
        %end
...
%        iGenerateSchedulerFiles(model, sys, prj_dir, xcp_bt);
        iGenerateSchedulerFiles(gcs_new, sys, prj_dir, xcp_bt);
    ...
    case {'build', 'buildrxe', 'buildrom', 'buildram'} 
        if xcp_bt == 1
           ...
           sys= nxtbuild(sys, 'cgen'); % call this function recursively
           ert_rtw_dir = [sys '_ert_rtw'];
        end
